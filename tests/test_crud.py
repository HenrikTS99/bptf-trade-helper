...
from tests.fake_bp import bp_listing_dict
...
async def test_upsert_listing_creates(db_session):
    ...
    await crud.upsert_listing(
        db_session, BPListing.from_api(bp_listing_dict(id="L2")), item.id
    )
...
async def test_upsert_listing_unchanged(db_session):
    _, item = await seed_listing(db_session)
    bp_listing = BPListing.from_api(bp_listing_dict(id="L2"))
...
async def test_upsert_listing_updates_price(db_session):
    ...
    changed = BPListing.from_api(bp_listing_dict(id="L2")).model_copy(
...
async def test_get_stored_buyorder_states_filter(db_session):
    listing, _ = await seed_listing(db_session, "L1")
    await seed_buyorder_state(db_session, listing, is_outbid=True)
    listing, _ = await seed_listing(db_session, "L2")
    await seed_buyorder_state(db_session, listing, is_outbid=False)
...
async def test_get_stored_sellorder_states_filter(db_session):
    listing, _ = await seed_listing(db_session, "L1")
    await seed_sellorder_state(db_session, listing, is_undercut=True)
    listing, _ = await seed_listing(db_session, "L2")
    await seed_sellorder_state(db_session, listing, is_undercut=False)
